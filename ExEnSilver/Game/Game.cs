...
namespace Microsoft.Xna.Framework
{
	public partial class Game : Canvas, IDisposable
	{
		public Game()
		{
			...
			// Set up a handler to change the Silverlight settings for timing...
			this.TimingValuesChanged += () =>
				{
					var settings = Application.Current.Host.Settings;

					if(!IsFixedTimeStep)
						settings.MaxFrameRate = 999;
					else
						settings.MaxFrameRate = (int)(TimeSpan.TicksPerSecond / gameLoop.TargetElapsedTime.Ticks);
				};
		}
